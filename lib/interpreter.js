...
class Interpreter {
  ...
  executeWhileStatement(statement, scope) {
    while (true) {
      let conditionResult = this.evalExpression(statement.condition, scope);
      ...
      if (result) {
        if (result.type === BREAK_STATEMENT_RESULT) {
          break;
        } else if (result.type === RETURN_STATEMENT_RESULT){
          throw result;
        }
      }
    }
  ...
  executeIFStatement(statement, scope) {
    let testResult = this.evalBinaryExpression(statement.test, scope);
    if (testResult) {
  ...
  evalCallExpression(expression, scope) {
    ...
    if (symbol === null) {
      ...
      if (gfunc === null) {
        ...
        let args = this.computeArguments(expression.arguments, scope);
        return gfunc.apply(null, args);
      }
    } else {
      let ast = symbol.node;
      let args = this.computeArguments(expression.arguments, scope);
      let funcScope = new Scope({
        super: ast.body.scope
      });
      let params = ast.params;
      params && params.forEach(function (param, idx) {
        funcScope.define(param.name, args && args[idx], param);
      });
      let result;
      try {
        result = this.executeBlockStatement(ast.body, funcScope);
      } catch (r) {
        if (r instanceof Error) {
          throw r
        }
  ...
  executeBlockStatement(statement, scope) {
    scope = scope || statement.scope;
    return this.executeStatementList(statement.body, scope);
  }
  ...
  executeStatementList(statmentList, scope) {
    let result;
    for (let i = 0, len = statmentList ? statmentList.length : 0; i < len; i++) {
      let statement = statmentList[i];
